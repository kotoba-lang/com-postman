(ns postman.main
  "Kotodama WASM entrypoint for the Postman clean-room actor (L4) — Clojure port.
  ...
  facts (`postman.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "postman")
(def tier "L4")
...
(def entity-specs
  [{:entity "Service"   :plural "services"   :id-prefix "postman_ser"
    :fields [:name :environment :language]        :required [:name :environment]
    :coerce {}                       :refs {}}
   {:entity "Error"      :plural "errors"      :id-prefix "postman_err"
    :fields [:serviceId :message :culprit :count] :required [:message :culprit]
    :coerce {:count :int}            :refs {:serviceId "Service"}}
   {:entity "Trace"     :plural "traces"     :id-prefix "postman_tra"
    :fields [:serviceId :operation :durationMs :status] :required [:operation :durationMs]
    :coerce {:durationMs :int}       :refs {:serviceId "Service"}}
   {:entity "Metric"    :plural "metrics"    :id-prefix "postman_met"
    :fields [:serviceId :name :value :unit] :required [:name :value]
    :coerce {:value :float}          :refs {:serviceId "Service"}}
   {:entity "Alert"     :plural "alerts"     :id-prefix "postman_ale"
    :fields [:serviceId :condition :severity :active] :required [:condition :severity]
    :coerce {:active :bool}          :refs {:serviceId "Service"}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "postman_inc"
    :fields [:title :status :severity :startedAt] :required [:title :status]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"postman.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "postman-compat" :tier tier :entities entities} 200])
...
(defn start! [] :postman-compat/ready)
